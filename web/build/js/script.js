...
(function($) {
	...
	if($('.course-inner-content h4 span').length > 0) {
		$('.course-inner-content h4 span, .rate-head span, .rate-head-five h2 span').counterUp({
            delay: 15,
            time: 1500
        });
	}
    ...
    if ($('.testimonial-five.lazy').length > 0) {
        $(".testimonial-five.lazy").slick({
            lazyLoad: 'ondemand',  // تحميل الصور عند الحاجة
            slidesToShow: 1,       // عرض عنصر واحد في كل مرة
            slidesToScroll: 1,     // التمرير بعنصر واحد
            autoplay: true,        // تفعيل التشغيل التلقائي
            autoplaySpeed: 3000,   // مدة ظهور كل عنصر قبل الانتقال
            speed: 1000,          // سرعة الانتقال بين العناصر
            dots: true,           // عرض النقاط للتنقل بين التقييمات
            arrows: false,        // إخفاء الأسهم
            infinite: true        // جعل السلايدر يعمل بشكل دائري
        });
    }
	...
	if($('.owl-carousel.instructors-course').length > 0 ){
		var owl = $('.owl-carousel.instructors-course');
	      	owl.owlCarousel({
                margin: 50, // تباعد بين العناصر
                stagePadding:30, // بديل عن padding

	        nav : false,
	        nav: true,
	        loop: true,
            autoplay: true, // تشغيل تلقائي
        autoplayTimeout: 3000, // مدة البقاء قبل الانتقال
	...
	// Slick testimonial three
        ...
        if ($('.mentor-testimonial.lazy').length > 0) {
            $(".mentor-testimonial.lazy").slick({
                lazyLoad: 'ondemand',
                infinite: true,
                rtl: $("html").attr("dir") === "rtl", // تفعيل RTL إذا كانت الصفحة RTL
                slidesToShow: 1,
                slidesToScroll: 1,
                autoplay: true,
                autoplaySpeed: 3000,
